...
/**
 * Servlet implementation class PetHotelEditServlet
 */
@WebServlet("/phEdit.hj")
public class PetHotelEditServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PetHotelEditServlet() {
        super();
...
int maxSize = 1024*1024*10; // 10MB
		...
		int phno = Integer.parseInt(mrequest.getParameter("phno"));
		...
		// 7. 서비스 결과 처리
		int result = new ManagerService().editpetHotel(ph);
	
		if(result > 0) {
			response.sendRedirect("phDetail.hj?phno=" + phno);
		} else {
			request.setAttribute("mag", "업체 수정 실패!");
			request.getRequestDispatcher("views/common/errorPage.jsp").forward(request, response);
		}
